...
if (isset($_POST['hapuskeluar'])) {
    ...
    $row = mysqli_fetch_assoc(mysqli_query(
        $conn,
        "SELECT * FROM keluar WHERE idkeluar='$idkeluar'"
    ));
    if ($row) {
        // Kembalikan stok
...
if (isset($_POST['addbarangkeluar'])) {
    ...
    // Cek stok cukup
    $cekStok = mysqli_fetch_assoc(mysqli_query(
        $conn,
        "SELECT stock FROM stock WHERE idbarang='$idbarang'"
    ));
    if ($cekStok['stock'] < $qty) {
        // Redirect dengan pesan error
        header("Location: keluar.php?error=stok_kurang");
        exit;
    }

    mysqli_query($conn, "INSERT INTO keluar (idbarang, qty, penerima, tanggal)
...
if (isset($_POST['updatekeluar'])) {
    ...
    if ($old) {
        $diff = $newQty - $old['qty']; // selisih tambahan keluar
        mysqli_query($conn, "UPDATE keluar SET idbarang='$idbarang', qty='$newQty',
                             penerima='$penerima' WHERE idkeluar='$idkeluar'");
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    $where = '';
    if ($dari && $sampai)
        $where = "AND k.tanggal BETWEEN '$dari' AND '$sampai'";
    elseif ($dari)
        $where = "AND k.tanggal >= '$dari'";
    elseif ($sampai)
        $where = "AND k.tanggal <= '$sampai'";

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="barang_keluar_' . date('Ymd_His') . '.csv"');
    $out = fopen('php://output', 'w');
    fputs($out, "\xEF\xBB\xBF");
    fputcsv($out, ['No', 'Tanggal', 'Nama Barang', 'Jumlah', 'Penerima']);
    $rows = mysqli_query(
        $conn,
        "SELECT k.*, s.namabarang FROM keluar k
         JOIN stock s ON s.idbarang = k.idbarang
         WHERE 1=1 $where ORDER BY k.tanggal DESC"
    );
    $i = 1;
    while ($r = mysqli_fetch_assoc($rows))
        fputcsv($out, [$i++, $r['tanggal'], $r['namabarang'], $r['qty'], $r['penerima']]);
    fclose($out);
    exit;
}
...
<body class="sb-nav-fixed">
                ...
                <div class="container-fluid px-4">
                    ...
                    <?php if (isset($_GET['error']) && $_GET['error'] === 'stok_kurang'): ?>
                        <div class="alert alert-danger alert-dismissible fade show no-print" role="alert">
                            <strong>Gagal!</strong> Stok barang tidak mencukupi untuk jumlah yang diminta.
                        ...
                        <div class="card-body py-2">
                            <form method="get" class="row g-2 align-items-end">
                                ...
                                <div class="col-auto">
                                    ...
                                    <a href="keluar.php" class="btn btn-sm btn-secondary ms-1">Reset</a>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                    ...
                                    $query = mysqli_query(
                                        $conn,
                                        "SELECT k.*, s.namabarang, s.stock, s.idbarang as sid
                                FROM keluar k
                                JOIN stock s ON s.idbarang = k.idbarang
                                WHERE 1=1 $where
                                    ...
                                    while ($data = mysqli_fetch_assoc($query)):
                                        ...
                                        <tr>
                                            ...
                                            <td class="no-print">
                                                <!-- Detail -->
                                                <button type="button"
                                                    class="btn btn-sm btn-info text-white btn-detail-keluar"
                                                    data-bs-toggle="modal" data-bs-target="#detailModalKeluar"
                                                    data-id="<?= $idkeluar ?>"
                                                    data-tanggal="<?= htmlspecialchars($data['tanggal']) ?>"
                                                    data-barang="<?= htmlspecialchars($data['namabarang'], ENT_QUOTES) ?>"
                                                    data-qty="<?= $data['qty'] ?>"
                                                    data-penerima="<?= htmlspecialchars($data['penerima'], ENT_QUOTES) ?>"
                                                    data-stock="<?= $data['stock'] ?>">
                                                    <i class="fas fa-eye"></i> Detail
                                                </button>
                                                <!-- Edit -->
                                                <button type="button" class="btn btn-sm btn-warning btn-edit-keluar"
                                                    data-bs-toggle="modal" data-bs-target="#editModalKeluar"
                                                    data-id="<?= $idkeluar ?>" data-barang="<?= $data['sid'] ?>"
                                                    data-qty="<?= $data['qty'] ?>"
                                                    data-penerima="<?= htmlspecialchars($data['penerima'], ENT_QUOTES) ?>">
                                                ...
                                                <form method="post" class="d-inline"
                                                    onsubmit="return confirm('Hapus data keluar ini?');">
                                                    <input type="hidden" name="idkeluar" value="<?= $idkeluar ?>">
                                                    <button type="submit" name="hapuskeluar" class="btn btn-sm btn-danger">
    ...
    <!-- ══════════════════  AREA PRINT  ══════════════════════════════════════════ -->
    <div id="printArea">
        <h3 style="text-align:center">Laporan Barang Keluar — ditss kicau mania</h3>
            ...
            WHERE 1=1 $where ORDER BY k.tanggal DESC"
                ...
                while ($pd = mysqli_fetch_assoc($printQ)):
                    ...
                    <tr>
                        ...
                        <td><?= $pd['qty'] ?></td>
                        <td><?= htmlspecialchars($pd['penerima']) ?></td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i>Tambah Barang Keluar</h5>
                ...
                <form method="post">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select name="barangnya" class="form-select" required>
                                ...
                                $sel = mysqli_query(
                                    $conn,
                                    "SELECT * FROM stock WHERE stock > 0 ORDER BY namabarang ASC"
                                );
                                while ($r = mysqli_fetch_assoc($sel)):
                                    ?>
                                    <option value="<?= $r['idbarang'] ?>">
                                        <?= htmlspecialchars($r['namabarang']) ?> (Stok: <?= $r['stock'] ?>)
                                    </option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="addbarangkeluar" class="btn btn-primary">
    ...
    <!-- ════════════════════  MODAL: EDIT  ═══════════════════════════════════════ -->
    <div class="modal fade" id="editModalKeluar" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit me-2"></i>Ubah Barang Keluar</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form method="post">
                    <div class="modal-body">
                        <input type="hidden" id="edit_idkeluar" name="idkeluar">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nama Barang</label>
                            <select id="edit_barangnya_keluar" name="barangnya" class="form-select" required>
                                <?php
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Jumlah</label>
                            <input type="number" id="edit_qty_keluar" name="qty" class="form-control" min="1" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Penerima</label>
                            <input type="text" id="edit_penerima" name="penerima" class="form-control" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" name="updatekeluar" class="btn btn-warning">
    ...
    <!-- ═══════════════════  MODAL: DETAIL  ══════════════════════════════════════ -->
    <div class="modal fade" id="detailModalKeluar" tabindex="-1">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title"><i class="fas fa-info-circle me-2"></i>Detail Barang Keluar</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body" id="detailBodyKeluar">
                    <!-- Diisi via JS -->
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        document.querySelectorAll('.btn-edit-keluar').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('edit_barangnya_keluar').value = this.dataset.barang;
                document.getElementById('edit_qty_keluar').value = this.dataset.qty;
                document.getElementById('edit_penerima').value = this.dataset.penerima;
            });
        });

        // ── Isi modal Detail ──────────────────────────────────────────────────────────
        document.querySelectorAll('.btn-detail-keluar').forEach(btn => {
            btn.addEventListener('click', function () {
                const body = document.getElementById('detailBodyKeluar');
                        ...
                        <table class="table table-borderless mb-0">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><i class="fas fa-calendar-alt me-1 text-danger"></i>${this.dataset.tanggal}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <th class="text-muted">Jumlah Keluar</th>
                                    <td>
                                        <span class="fs-5 fw-bold text-danger">
                                            <i class="fas fa-minus-circle me-1"></i>${this.dataset.qty}
                                        </span>
                                    </td>
                                </tr>
                                <tr>
                                    <th class="text-muted">Penerima</th>
                                    <td>${this.dataset.penerima || '<em class="text-muted">-</em>'}</td>
                                </tr>
                                <tr>
